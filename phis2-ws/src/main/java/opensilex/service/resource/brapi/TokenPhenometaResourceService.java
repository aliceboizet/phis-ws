...
import opensilex.service.authentication.TokenPhenometaResponseStructure;
 ...
 */
@Api(value = "/brapi/v1/tokenPhenometa")
@Path("brapi/v1/tokenPhenometa")
@Singleton
public class TokenPhenometaResourceService {
    
    final static Logger LOGGER = LoggerFactory.getLogger(TokenPhenometaResourceService.class);
    ...
    @ApiOperation(value = "Login",
            ...
            response = TokenPhenometaResponseStructure.class)
    @ApiResponses(value = {
    ...
    public Response getTokenPhenometa(@ApiParam(value = "JSON object needed to login") @Valid TokenDTO jsonToken, @Context UriInfo ui) {
        ...
        if ((password != null && username != null) || (isJWT && validJWTToken && username != null)) {
            ...
            try {
                ...
                if (user == null) {
                    ...
                    // return result
                    TokenPhenometaResult result = new TokenPhenometaResult(user.getFirstName() + " " + user.getFamilyName(), userSessionId, expires_in);
                    TokenPhenometaResponseStructure response = new TokenPhenometaResponseStructure(result);
                    final URI uri = new URI(ui.getPath());
                    return Response.status(responseStatus).location(uri).entity(response).build();
                }
